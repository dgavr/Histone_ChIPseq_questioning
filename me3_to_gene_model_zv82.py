...
#        exonFrames =line.split()[16]    #Exon frame offsets {0,1,2}
        introns = [i - j for i, j in zip(exonStarts, exonEnds)]        
        if strand == '+': 
            if chrom in gene_pred_dict_plus: 
                gene_pred_dict_plus[chrom].append(str(txStart)+'-'+str(txEnd)+'-'+str(Ensdart_id))
            else: 
                gene_pred_dict_plus[chrom] = []
                gene_pred_dict_plus[chrom]= [txStart+'-'+txEnd+'-'+Ensdart_id]
        if strand == '-': 
            if chrom in gene_pred_dict_minus: 
                gene_pred_dict_minus[chrom].append(str(txStart)+'-'+str(txEnd)+'-'+str(Ensdart_id))
            else: 
                gene_pred_dict_minus[chrom] = []
                gene_pred_dict_minus[chrom]= [txStart+'-'+txEnd+'-'+Ensdart_id]
    return gene_pred_dict_plus, gene_pred_dict_minus, introns
