...
def find_1st_line():
    ...
    for line in wigfile: 
        if line.startswith('track'):
            first_line = 'track type=wiggle_0 name=33pc_'+line.split()[2].split('=')[1]+'maxHeightPixels=64:64:11 color=31,120,180 visibility=full \n'
            break
    wigfile.close()
    return first_line
...
if __name__ == "__main__":
    ...
    out_wig_filename = 'Minus_33pc_wig_'+wigfilename.split('.')[0]+'.wig'
    out_bed_filename = 'Minus_33pc_'+wigfilename.split('.')[0]+'.bed'
    ...
    out_bed.write('track name='+'Minus_33pc' + wigfilename.split('.')[0] + '\n')
    for declaration_track, peak_intensity in read_wig(wig): #for each old 'peak'
        ...
        threshold = np.mean(peak_intensity)/3
