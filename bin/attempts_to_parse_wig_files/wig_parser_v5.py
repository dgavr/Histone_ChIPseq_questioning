...
###############################################################################
#Goal: reads a wig file and generates txt file with statistics of length and 
# peak intensity of the peaks
################################################################################
import sys
import numpy as np
...
if __name__ == "__main__":
    ...
    out_filename = 'Stats_per_chr_'+wigfilename.split('.')[0]+'.txt'
    wig = open(wigfilename,'r') #wig input handler
    out_file = open(out_filename, 'w')   
    out_file.write('Chromosome'+'\t'+'Total_intensity'+'\t'+'Length_of_peak'+'\t'+'Distance_to_prev_peak'+'\n')
    prev_stop= None
    old_chrom= None
    for declaration_track, peak_intensity in read_wig(wig):
        ...
        out_file.write(str(chrom)+'\t'+str(sum_int)+'\t'+str(len_peak)+'\t'+str(dist)+'\n')
        old_chrom = chrom
