...
def get_height_of_peaks(list_peaks, list_hm1, list_hm2):
    """
    (list)->(list)
    """
    list_peak_intensity =[]
    for x in list_peaks: 
        hm1_int = list_hm1[x+1]
        hm2_int = list_hm2[x+1]
        list_peak_intensity.append(((hm1_int, hm2_int)))
    return list_peak_intensity
...
if __name__ == "__main__":
    ...
    for chrom in size_dictionary:
        ...
        for pos in common_peaks: 
                print 'pos is ', type(pos)
                output_peak_file.write('\t'.join([str(chrom), str(pos+1), str(pos+2)])+'\n')
        window_counter+=1    
